...
const Post = () => {
    const breadcrumbItems = [
        ...
        { title: <AntdComponents.Typography className='active'>Post</AntdComponents.Typography> }
    ];
    ...
    const { data: listData, isLoading } = useDynamicQueryQuery({
        endpoint: Endpoints.NON_AUTH.REPORT_POST_LIST,
        params: { page: pagination?.current, limit: pagination?.pageSize },
        key: QueryKeys.REPORT_POST_LIST,
        skip: false,
    })
    ...
    const columns = [
        {
            ...
            render: (data) =>
                <AntdComponents.Typography className='font-bold capitalize text-White' onClick={()=> navigate(`${RouterKeys?.NON_Auth.USER_Detail}?id=${data?._id}`) }>{data?.name ? data?.name : "---"}</AntdComponents.Typography>,
        ...
        },
        {
            ...
            dataIndex: 'post',
            key: 'post',
            render: (data) => <AntdComponents.Typography className='table_row_data'>{data?.description ? data?.description : "---"}</AntdComponents.Typography>,
            ...getSorterConfigForObject('post', 'alpha', 'description'),
            minWidth: 150,
        },
        {
            title: 'Action',
            ...
            render: (Id, record) => (
                <div className="flex gap-2">
                    <AntdComponents.Avatar
                        ...
                        onClick={() => { navigate(`${RouterKeys.NON_Auth.REPORT_POST_DETAIL}?id=${Id}`) }}
                    />
                </div>
            ),
            width: 100,
